...
const Mapnificent = (function () {
  ...
  MapnificentPosition.prototype.updateProgress = function (percent) {
    ...
    if (percent === undefined) {
      var max =
        this.mapnificent.settings.options.estimatedMaxCalculateCalls || 100000;
  ...
  MapnificentPosition.prototype.createWorker = function () {
    ...
    this.webworker = new window.Worker(
      this.mapnificent.settings.baseurl + "mapnificentworker.js"
    );
    /* new end */
  ...
  MapnificentPosition.prototype.getReachableStations = function (
    ...
    var getLngRadius = function (lat, mradius) {
      var equatorLength = 40075017,
        hLength = equatorLength * Math.cos(DEG_TO_RAD * lat);
  ...
  function Mapnificent(map, city, pageConfig) {
    ...
    this.settings.options = Object.assign(
      {
        maxWalkTravelTime: 1.5 * 60 * 60
      },
      this.settings.options
    );
    this.settings = Object.assign(this.settings, pageConfig);
  }
    ...
    return this.loadData().then(function (data) {
      ...
      if (self.settings.coordinates) {
        ...
        if (self.positions.length === 0) {
          self.addPosition(
            L.latLng(self.settings.coordinates[1], self.settings.coordinates[0])
          );
        }
      }
    });
  ...
  Mapnificent.prototype.logDebugMessage = function (latlng) {
    var self = this;
    var stationsAround = this.quadtree.searchInRadius(
      latlng.lat,
      latlng.lng,
      300
    );
    this.positions.forEach(function (pos, i) {
      ...
      stationsAround.forEach(function (station) {
        var lastTransport;
        ...
        pos.debugMap[station.id].forEach(function (stop, k) {
          ...
          if (lastTransport != stop.line) {
            console.log(
              k,
              "Switching transport to",
              self.lineNames[stop.line],
              "waiting: ",
              stop.waittime
            );
          }
  ...
  Mapnificent.prototype.loadData = function () {
    ...
    const MAPNIFICENT_PROTO = {
      nested: {
        mapnificent: {
          nested: {
            MapnificentNetwork: {
              fields: {
                Cityid: { type: "string", id: 1 },
                Stops: { rule: "repeated", type: "Stop", id: 2 },
                Lines: { rule: "repeated", type: "Line", id: 3 }
              },
              nested: {
                Stop: {
                  fields: {
                  ...
                  nested: {
                    TravelOption: {
                      fields: {
                        Stop: { type: "uint32", id: 1 },
                        TravelTime: { type: "uint32", id: 2 },
                        StayTime: { type: "uint32", id: 3 },
                        Line: { type: "string", id: 4 },
                        WalkDistance: { type: "uint32", id: 5 }
                      }
                    }
                  }
                },
                Line: {
                  fields: {
                    LineId: { type: "string", id: 1 },
                    LineTimes: { rule: "repeated", type: "LineTime", id: 2 },
                    Name: { type: "string", id: 3 }
                  },
                  nested: {
                    LineTime: {
                      fields: {
                        Interval: { type: "uint32", id: 1 },
                        Start: { type: "uint32", id: 2 },
                        Stop: { type: "uint32", id: 3 },
                        Weekday: { type: "uint32", id: 4 }
                      }
                    }
                  }
                }
    ...
    return new Promise((resolve) => {
      ...
      oReq.onload = function (oEvent) {
        var MapnificentNetwork = protoRoot.lookupType(
          "mapnificent.MapnificentNetwork"
        );
        console.log("received binary", new Date().getTime());
        var message = MapnificentNetwork.decode(
          new Uint8Array(oEvent.target.response)
        );
  ...
  Mapnificent.prototype.getLineTimesByInterval = function (lineTimes) {
    ...
    for (var i = 0; i < lineTimes.length; i += 1) {
      result[lineTimes[i].Weekday + "-" + lineTimes[i].Start] =
        lineTimes[i].Interval;
    }
    return result;
  };
  ...
  Mapnificent.prototype.prepareData = function (data) {
    ...
    for (i = 0; i < data.Lines.length; i += 1) {
      if (!data.Lines[i].LineTimes[0]) {
        continue;
      }
      this.lines[data.Lines[i].LineId] = this.getLineTimesByInterval(
        data.Lines[i].LineTimes
      );
      if (this.settings.debug) {
    ...
    this.quadtree = Quadtree.create(
      ...
      this.settings.bounds[2],
      this.settings.bounds[3]
    );
    ...
    var parseHash = function (hash) {
      ...
      for (var i = 3; i < args.length; i += 3) {
        ...
        if (mapnificent.positions[posIndex] === undefined) {
          ...
          mapnificent.positions[posIndex].updatePosition(
