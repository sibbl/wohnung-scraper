...
module.exports = class AbstractScraper {
    constructor(db, scraperId) {
        ...
        if (typeof this.scrapeItemDetails !== "function") {
            throw new TypeError(
                "Scraper must override method scrapeItemDetails."
            );
        }
    ...
    async insertIntoDb(row) {
        try {
            ...
            throw new Error(
                `Error while inserting into database (ID=${
                    this.id
                }, error: ${e}`
            );
        }
        ...
        try {
            return await this.statements.update.run({
                ...
                $removed:
                    row.removed == null
                        ? null
                        : moment(row.removed).toISOString(),
                $comment: row.comment,
    ...
    async hasItemInDb(id) {
        try {
            ...
            throw new Error(
                `Error while checking existing items (ID=${
                    this.id
                }, error: ${e}`
            );
        }
    ...
    sendBotNotifications(bots, result) {
        ...
        var flatsOfInterest = result.filter(result => {
            ...
            if (
                flat.price < filters.price.min ||
                flat.price > filters.price.max
            ) {
                return false;
            }
            if (
                flat.rooms < filters.rooms.min ||
                flat.rooms > filters.rooms.max
            ) {
                return false;
            }
